...
local dialogArr = {"Закурить сигарету (на улице)", "Выкинуть и потушить сигарету (на улице)", "Закурить сигарету в помещении (стоя) {ff0000}Не готово", "Выкинуть и потушить сигарету в помещении (стоя) {ff0000}Не готово", "Закурить сигарету в помещении (сидя)", "Выкинуть и потушить сигарету в помещении (сидя с пепельницей)", "Отыгровка маски-респиратора черного цвета на лице (Обезболивающий газ)", "Карманник (Забрать кошелек из переднего кармана и скрыться в толпе)", "Карманник (Забрать кошелек из заднего кармана и скрыться)"}
local dialogStr = ""
		...
		if result then
			...
			if button == 1 then 
				if list == 0 then -- на улице 0
					...
					wait(7000)
					sampSendChat(u8:decode("/me достал из кармана фирменную зажигалку \"" .. mainIni.config.brandedLighter .. "\" и подставил её к сигарете, зажимая кнопку поджига."))
					wait(3500)
				...
				if list == 4 then -- сидя в помещении 1 
					...
					wait(7000) 
					sampSendChat(u8:decode("/me вынимает фирменную зажигалку \"" .. mainIni.config.brandedLighter .. "\" из кармана и подносит ее к сигарете, нажимая кнопку поджига.")) 
					wait(2500) 
					sampSendChat(u8:decode("/do Сигарета прикурена.")) 
					wait(2000) 
				...
				if list == 6 then -- маска-респиратор
					mainIni = inicfg.load(nil, directIni)
				...
				if list == 7 then -- карманник (забрать кошелек из переднего кармана)
					mainIni = inicfg.load(nil, directIni)
					sampSendChat(u8:decode("/me остановился возле прохожего и внимательно посмотрел ему в глаза, стоя на его правой стороне."))
					wait(5500) 
					sampSendChat(u8:decode("/me мастерски проскользнул рукой в его карман, незаметно вытащил кошелек и быстро извлек оттуда деньги."))
					wait(3500) 
					sampSendChat(u8:decode("/me спрятав деньги, аккуратно и тихо вернул кошелек в тот же карман, чтобы не вызвать подозрений."))
					wait(4500)
					sampSendChat(u8:decode("/me быстро ускользнул в тень и скрылся от глаз прохожего."))
					wait(3500)
					sampSendChat(u8:decode("/do " .. mainIni.config.nameNominativeCase .. " слился с окружением, чтобы не вызывать подозрений и быть незаметным"))
					wait(1500)
					sampSendChat(u8:decode("/me чувствовал удовлетворение и адреналин от успешно выполненного обмана."))
				end

				if list == 8 then -- карманник (забрать кошелек из заднего кармана и скрыться)
					mainIni = inicfg.load(nil, directIni)
					sampSendChat(u8:decode("/me приближается к жертве, аккуратно становится позади и начинает медленно вытаскивать кошелек из ее заднего кармана."))
					wait(5500) 
					sampSendChat(u8:decode("/do " .. mainIni.config.nameNominativeCase .. " внимательно следит за ситуацией, чтобы никто не заметил его деликатного маневра. Он уверенно работает, не теряя концентрации."))
					wait(3500) 
					sampSendChat(u8:decode("/me успешно вытаскивает кошелек и быстро прячет его в своем кармане, убедившись, что никто не заметил его действия."))
					wait(4500)
					sampSendChat(u8:decode("/me зловеще усмехается и безмятежно покидает место преступления, с уверенностью в своем мастерстве и ..."))
					wait(1000)
					sampSendChat(u8:decode("/do ... знании, что добыча теперь находится в его руках."))
				end
			end
		end
